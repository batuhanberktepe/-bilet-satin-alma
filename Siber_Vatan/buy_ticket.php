...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Düzeltme burada: Hatalı PHP bloğu kaldırıldı.
    // Orijinal dosyadaki bozuk PHP `if` bloğu (satır 245 civarı)
    // JavaScript'in geri kalanının çalışmasını engelliyordu.
    // Artık 'if' bloğu yok ve kod doğru çalışacak.

    <?php if ($error_message && !empty($coupon_code)): ?>
    // Bu PHP bloğu, sayfa YENİDEN YÜKLENDİĞİNDE çalışır (POST işleminden sonra)
    // ve kupon giriş alanını tekrar aktif hale getirir.
    couponInput.readOnly = false;
    applyCouponBtn.disabled = false;
    applyCouponBtn.innerText = 'Kuponu Uygula';
    <?php endif; ?>
